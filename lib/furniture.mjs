...
export function buildFurniture(spec, floors, upperHeightAt) {
  ...
  for (const floor of [1, 2]) {
    ...
    for (const sourceItem of spec.furniture?.items ?? []) {
      const item = { ...sourceItem, position: [...sourceItem.position] };
      if (item.kind === "air-conditioner" && item.openingId) {
        const wall = spec.walls.find((w) => w.openings?.some((o) => o.id === item.openingId));
        if (!wall) throw new Error(`${item.id}: unknown opening ${item.openingId}`);
        const opening = wall.openings.find((o) => o.id === item.openingId);
        const dx = wall.b[0] - wall.a[0];
        const dz = wall.b[1] - wall.a[1];
        const length = Math.hypot(dx, dz);
        const along =
          ((item.position[0] - wall.a[0]) * dx + (item.position[1] - wall.a[1]) * dz) / length;
        const shift = opening.start + opening.width / 2 - along;
        item.position[0] += (shift * dx) / length;
        item.position[1] += (shift * dz) / length;
      }
