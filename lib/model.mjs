...
import { buildElectrical } from "./electrical.mjs";
import { buildFurniture } from "./furniture.mjs";
import {
  garageLayout,
  garageWalls,
  stairLayout,
...
export function upperHeightAt(x, z, p, zone = "auto") {
  ...
  const south =
    zone === "south" ||
    (zone === "auto" && z >= (p.upperSouthStart ?? 7.5) && x <= (p.upperSouthWidth ?? 5.19) + 0.6);
  let edge = south ? (p.upperSouthWidth ?? 5.19) : (p.upperMainWidth ?? 8.82);
  ...
  const right =
    knee +
    (Math.max(0, edge - x) * (p.technicalUpperHeight - knee)) /
      (south ? (p.southRightSlopeRun ?? p.rightSlopeRun) : p.rightSlopeRun);
...
export function buildHouse(spec) {
  ...
  const outline1 = spec.envelope?.floor1 ?? [
    [-0.6, -0.6],
    [9.42, -0.6],
    [9.42, 8.02],
    [5.85, 8.02],
  ...
  const outline2 = spec.envelope?.floor2 ?? [
    [-0.6, -0.6],
    [9.42, -0.6],
    [9.42, 7.99],
    [5.79, 7.99],
  ...
  const g = garage,
    t = g.wallThickness;
  // The structural slab continues below the solid wall projection. Offset its
  // outer envelope, not the narrow recess (which would self-intersect at 0.3 m).
  const garageSlab = g.perimeterWalls
    ? offsetPolygon(
        [
          g.frontLeft,
          g.backLeft,
  ...
  plinth(
    [
      ...
      [g.left, g.shortBack],
      [g.left, g.leftSharedWallEndZ + 0.6],
      [g.left - 0.6, g.leftSharedWallEndZ + 0.6],
    ],
    "GARAGE-LEFT-PLINTH",
  );
  plinth(
  ...
  for (const w of [...spec.walls, ...garageWalls(spec)]) {
    ...
    if (w.floor === 2) {
      ...
      for (const x of [
        runL,
        (p.upperMainWidth ?? 8.82) -
          ((p.upperHeight - p.rightKneeHeight) * p.rightSlopeRun) /
            (p.technicalUpperHeight - p.rightKneeHeight),
        (p.upperSouthWidth ?? 5.19) -
          ((p.upperHeight - p.southRightKneeHeight) * (p.southRightSlopeRun ?? p.rightSlopeRun)) /
            (p.technicalUpperHeight - p.southRightKneeHeight),
      ])
        if (Math.abs(ux) > 0.1) cuts.push((x - w.a[0]) / ux);
      if (Math.abs(uz) > 0.1) cuts.push(((p.upperSouthStart ?? 7.5) - w.a[1]) / uz);
    }
  ...
  // Keep the scan-derived garage projection as its own wall, but overlap its
  // attachment with the existing house wall so the shared face has no seam.
  if (g.wallProjection?.joinDepth) {
    const [rearLeft, frontLeft, frontRight] = g.wallProjection.vertices.map(
      (i) => g.room.polygon[i],
    );
    const width = Math.abs(frontRight[0] - frontLeft[0]);
    const x = (frontLeft[0] + frontRight[0]) / 2 + (g.wallProjection.shiftX ?? 0);
    box(
      x,
      g.floorOffset + g.ceilingHeight / 2,
      rearLeft[1] - g.wallProjection.joinDepth / 2,
      width,
      g.ceilingHeight,
      g.wallProjection.joinDepth,
      materials.wall,
      "GARAGE-W05-JOIN",
      1,
      "wall",
      { wallId: "GARAGE-W05", component: "garage", join: true },
    );
  }
  ...
  for (const r of spec.rooms) {
    ...
    if (r.floor === 2) {
      const edge = r.id === "F2-ROOM" ? (p.upperSouthWidth ?? 5.19) : (p.upperMainWidth ?? 8.82);
      ...
      breaks.push(
        ...
        edge -
          ((p.upperHeight - knee) *
            (r.id === "F2-ROOM" ? (p.southRightSlopeRun ?? p.rightSlopeRun) : p.rightSlopeRun)) /
            (p.technicalUpperHeight - knee),
      );
    }
  ...
  // Existing divider closes the lower flight, flush with the upper flight underside.
  ...
  buildFurniture(spec, floors, upperHeightAt);
  buildElectrical(spec, floors);
