...
Два этажа, лестница, проёмы и скошенные потолки, без мебели. Геометрия восстановлена по обмерам. Высокая часть второго этажа — **2,97 м**, подтверждена владельцем 08.09.2026.
...
| `model-builder.mjs` | Общий генератор геометрии на Three.js                            |
...
Приоритет — **«черновой план.pdf»**, технический **«step6_ver3.pdf»** дополняет недостающие сведения. Общие планы обоих этажей сохранены в `public/plans/`. Полные PDF остаются у владельца и не включены в репозиторий.

Размеры помещений, координаты, управление просмотрщиком и все допущения описаны в [docs/MODEL.md](docs/MODEL.md). Толщина перекрытия, детали лестницы и линии переходов скосов требуют уточнения по дополнительным обмерам. Обзорное изображение выше — снимок исходной версии, не обновляемый автоматически.
